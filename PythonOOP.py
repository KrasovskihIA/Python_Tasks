...
"""
одвиг 4. Известно, что в Python мы можем соединять два списка между собой с помощью оператора +:
...
class ListMath:
    def __init__(self, lst=None):
        self.lst_math = lst if lst and type(lst) == list else [] # ссылается на lst если он существует и если это список, если нет создает пустой список
        self.lst_math = list(filter(lambda x: type(x) in (int, float), self.lst_math))

    @staticmethod
    def __verife_value(value):
        if type(value) not in (int, float):
            raise ArithmeticError("Неверный операнд")

    def __add__(self, other):
        self.__verife_value(other)
